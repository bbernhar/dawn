...
namespace dawn_native { namespace d3d12 {
    ...
    class PipelineCache {
      public:
        ...
        ResultOrError<ComPtr<ID3D12PipelineState>> GetOrCreate(const DescT& desc,
                                                               size_t descHash,
                                                               bool useCache) {
            ComPtr<ID3D12PipelineState> pso;
            const std::wstring& descHashW = std::to_wstring(descHash);
            if (mLibrary != nullptr && useCache) {
      ...
      private:
        ...
        PersistentCacheKey CreatePipelineCacheKey(const Adapter* adapter) const;
        ...
        size_t mCacheHitCountForTesting = 0;
